...
$(window).load(function() {
  ...
  var drawingTool = new DrawingTool("#drawing-tool02", {
    stamps: {
      _: [
    ...
    width: 500,
    height: 550,
    backgroundImage: "image/infographic-background.png"
    });
...
function saveState() {
  ...
  setUserVar('T540_DT_2', _drawingTool.save());
  setUserVar('T540_DT_2_image', _drawingTool.canvas.toDataURL('image/png'));
...
function getFTEData(formatted) {
  ...
  if (formatted) {
    _drawingTool.clearSelection();
    return '<img id="T540_DT_image_2" src="' + _drawingTool.canvas.toDataURL('image/png') + '"/>';
  } else {
    return state;
  }
}
